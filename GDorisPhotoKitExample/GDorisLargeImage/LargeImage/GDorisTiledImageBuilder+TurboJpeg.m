//
//  GDorisTiledImageBuilder+TurboJpeg.m
//  GDorisPhotoKitExample
//
//  Created by GIKI on 2019/9/17.
//  Copyright © 2019 GIKI. All rights reserved.
//

#import "GDorisTiledImageBuilder+TurboJpeg.h"
#import "GDorisTiledImageBuilderInternal.h"
#define LOG NSLog
...
@implementation GDorisTiledImageBuilder (TurboJpeg)
        ...
        for(size_t idx=0; idx<self.zoomLevels; ++idx, ++im) {
            ...
            if(ubc_usage > self.ubc_threshold) {
                if(OSAtomicCompareAndSwap32(0, 1, &fileFlushGroupSuspended)) {
                    // LOG(@"SUSPEND==============================================================================");
                    dispatch_suspend([GDorisTiledImageBuilder fileFlushQueue ]);
                    dispatch_group_async([GDorisTiledImageBuilder fileFlushGroup], [GDorisTiledImageBuilder fileFlushQueue ], ^{ LOG(@"unblocked!"); } );
                }
            }
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^
                           {
                               ...
                               if(ubc_usage <= self.ubc_threshold) {
                                   if(OSAtomicCompareAndSwap32Barrier(1, 0, &fileFlushGroupSuspended)) {
                                       dispatch_resume([GDorisTiledImageBuilder fileFlushQueue]);
                                   }
                               }
                           } );
        }
...
@implementation GDorisTiledImageBuilder(JPEG_PUB)
